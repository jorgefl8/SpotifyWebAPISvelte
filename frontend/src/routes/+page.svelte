<script>
    ...
    let redirect_uri = "http://localhost:12345/"
    ...
    <div class="container" style="margin-top: 1%;">
        {#if show_token_section == true}
            <div class="row" style="margin-bottom: 1%;">
                <h2>
                    List your spotify playlist using OAUTH2.0 
                </h2>
                <p>
                    For using this functionality, you have to go to the spotify developer page: 
                    <a href="https://developer.spotify.com/dashboard/applications">https://developer.spotify.com/dashboard/applications</a>,
                    create an APP to get the Client Id and Secret and add <strong>http://localhost:12345/spotify</strong> in the "Redirect URIs" field.
                </p>
            </div>
            <Form>
                <FormGroup>
            ...
            <div class="row">
                <div class="col-md-6">
                    <Table  bordered striped>
                        <thead>
                            <tr>
                                <th><h3 style="margin-right: 1%;">Playlists</h3> <Button style="margin-right: 1%;" color="warning" on:click={refreshPlaylists}>Reload</Button> <Button color="danger" on:click={revokeAcces}>Clean credentials</Button></th>
                            </tr>
                        </thead>
                        <tbody>
                            {#if playlists.length == 0}
                                <td colspan="6"><p class="text-center">Theres no data.</p></td>
                            {:else}
                                {#each playlists as x}
                                    <tr>
                                        <td>
                                            <img src={x.url_image} alt="Imagen de {x.name}" />
                                            {x.name} ({x.tracks})
                                        </td>
                                    </tr>
                                {/each}
